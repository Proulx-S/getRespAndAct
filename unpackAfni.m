...
for r = 1:size(fRes.fIn,1)
    ...
    if fRes.param.PCflag
        ...
        if strcmp(fIn,stats.fPoly0Base{r,1}); dbstack; error('fIn and fOut are the same'); end
        if strcmp(fIn,stats.fPoly0Base{r,2}); dbstack; error('fIn and fOut are the same'); end
        if force || ...
        ...
        if strcmp(fIn,fOut); dbstack; error('fIn and fOut are the same'); end
...
if ~any(contains(tmp,'part-')) || ~contains(tmp{contains(tmp,'part-')},'Mag1')
    ...
    for r = 1:size(fRes.fIn,1)
        ...
        if strcmp(fIn,fOut)
            dbstack; warning('fIn and fOut are the same, falling back to new name outside convention');
            fOut = replace(fIn,'.nii.gz','_av.nii.gz');
        end
